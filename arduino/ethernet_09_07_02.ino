...
// ----------------------------
// Execute a received sequence using the OPEN-LOOP sleeve-height model
// ported from motor_control_code.ino: for each waypoint, compute the
// height delta between the previous and this point's commanded throttle
// (via h_of_throttle()), convert that to a step count via GEARBOX_RATIO/
// STEPS_DIV, and run Motor for exactly that many steps at the rate needed
// to finish within this waypoint's duration_ms.
//
// This is executed as TIMED continuous stepping (start, wait the computed
// duration, stop) rather than an explicit step-counted loop, since Motor's
// protocol only supports "run continuously" / "stop" -- mathematically
// equivalent to running steps_to_take steps at the comp... (10 KB left)